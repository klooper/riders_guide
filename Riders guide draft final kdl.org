...
You can schedule trips yourself using the HCE Rides app without having to call Hamilton County Express reservations. Rides must be scheduled at least two hours prior to pickup. Follow these steps:
    ...
    *4. Click and review the four buttons above the Select Hamilton County Express button.*
        They are:
          ...
          + *Accessibility options:* Clicking on the top-right button allows you to ensure that all accessibility needs for the ride are accounted for. Click Confirm Accessibility to confirm your selections.
...
- *Accessibility Constraints:* There are a few constraints to the level of service that drivers can provide, as follows:
...
*** TODO Come to consensus about Late Cancellation and No Show policy 
...
*** TODO Do we need a definition and policy regarding sustained negative eWallet balance?
...
*** TODO We may need more of a definition of hazardous materials here. Bleach, for example, is a hazardous material, as are many household products.
...
*** TODO Come to consensus about policy for revoking subscription trips. In some cases (No-shows, late cancels, and so on), it seems like this should be the /first/ thing that happens. These may happen because of changes to traffic patterns, times, changing needs of HCE, etc., so I don't know that it should always be punitive. Maybe something to the effect that subscription trips are a privilege that are totally within the scope of HCE management to approve and continue based on the best interest of Hamilton County Express. See wording of Rider Conduct policy note.
...
*** TODO This is what Teresa told me to put in here, kind of. We really need to flesh out exactly what the appeals process is, who conducts the appeals, and what, for example, happens when we cannot contact the individual, such as a home address outside of Hamilton County. 14 days seems pretty long, as they will already be halfway into the next month before the previous suspension would go into effect. There's really nothing 
...
*** TODO Add formatting codes for Latex PDF export and link to that here.
